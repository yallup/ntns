#!/usr/bin/env python
"""NTNS-IRMH on LJ55 (55 particles, 3D, 165D total).

NTNS pattern: prior rejection -> EGNN flow trained on live points -> flow used
as an independent Metropolis-Hastings proposal (tsit5 ODE solver, Hutchinson
trace for the log-det at this dimension).
"""
...
from ntns import LennardJones, ParticleFlowMatching
...
NLIVE, NDELETE, NUM_INNER_STEPS = 4096, 1024, 40
SEED = 42

FLOW_EPOCHS, FLOW_BATCH, FLOW_LR, FLOW_STEPS = 300, 512, 1e-3, 25
FLOW_PROBES = 10  # Hutchinson trace probes (exact VJP too expensive at 165D)
...
flow = ParticleFlowMatching(
    ...
    hidden_nf=EGNN_HIDDEN, n_layers=EGNN_LAYERS, n_probes=FLOW_PROBES,
    rtol=1e-3, atol=1e-3,
)
