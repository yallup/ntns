#!/usr/bin/env python
"""NTNS-MALA on DW4 (4 particles, 2D, 8D total).

NTNS pattern with the MALA inner kernel: the EGNN flow's velocity field at
t_drift is used as the Langevin drift; step size adapted by Robbins–Monro from
the previous NS iteration's acceptance.
"""
...
from ntns import DoubleWell, ParticleFlowMatching, build_mala_kernel, MALAStepInfo

# Config -----------------------------------------------------------------
N_PARTICLES, N_DIM = 4, 2
DIM = N_PARTICLES * N_DIM  # 8

NLIVE, NDELETE, NUM_INNER_STEPS = 2048, 1024, DIM
SEED = 42

FLOW_EPOCHS, FLOW_BATCH, FLOW_LR, FLOW_STEPS = 150, 512, 1e-3, 25
EGNN_LAYERS, EGNN_HIDDEN = 5, 128

PRIOR_SIGMA = 1.0
PRIOR_FACTOR = 100

# MALA + Robbins–Monro step-size adaption
T_DRIFT = 0.5            # flow time at which to evaluate velocity for the drift
INIT_STEP_SIZE = 1e-3
RM_TARGET = 0.3          # below the 0.574 MALA optimum: NS constrained-prior
                         # geometry is non-product / boundary-dominated, so we
                         # trade acceptance for step size (better mixing per step)
RM_GAMMA = 0.5           # base gain on log-step
RM_KAPPA = 0.0           # 0 = constant gain (NS target is non-stationary)

OUTDIR = "results/dw4_mala"
...
dead_list = [NSInfo(dead_states, MALAStepInfo(
    acceptance_rate=jnp.array(NLIVE / n_prior),
    is_accepted=jnp.array(False),
))]
...
def mala_update_fn(rng_key, state, info, inner_kernel_params=None):
    """Retrain flow on current live set; RM-adapt step_size from prev acceptance."""
    prev_acc = float(info.update_info.is_accepted.mean()) if info is not None else None
    flow.train(
        state.particles.position,
        n_epochs=FLOW_EPOCHS, batch_size=FLOW_BATCH, lr=FLOW_LR,
        warm_start=True,
    )
    prev = inner_kernel_params or {}
    step_size = float(prev.get("step_size", INIT_STEP_SIZE))
    t = int(prev.get("adapt_t", 0))
    if prev_acc is not None:
        t += 1
        gain = RM_GAMMA / (t ** RM_KAPPA)
        step_size *= float(np.exp(gain * (prev_acc - RM_TARGET)))
    return {"step_size": step_size, "adapt_t": t, "flow_params": flow.params}


# Attach initial kernel params (avoids redundant retrain in adaptive_init's hook)
state = state._replace(inner_kernel_params={
    "step_size": INIT_STEP_SIZE,
    "adapt_t": 0,
    "flow_params": flow.params,
})

flow_kernel = build_mala_kernel(
    init_state_fn=init_state_fn,
    logdensity_fn=logprior_fn,
    net_apply=flow.net.apply,
    t_drift=T_DRIFT,
    project_fn=flow.remove_mean,
    num_inner_steps=NUM_INNER_STEPS,
    num_delete=NDELETE,
    update_inner_kernel_params_fn=mala_update_fn,
)

acc_rates, step_sizes = [], []
with tqdm.tqdm(desc="NTNS-MALA", unit=" pts") as pbar:
    while not (state.integrator.logZ_live - state.integrator.logZ < -3):
        ...
        acc = float(info.update_info.is_accepted.mean())
        step = float(state.inner_kernel_params["step_size"])
        acc_rates.append(acc)
        step_sizes.append(step)
        pbar.update(NDELETE)
        pbar.set_postfix(logZ=f"{float(state.integrator.logZ):.2f}", acc=f"{acc:.3f}", tau=f"{step:.2e}")
