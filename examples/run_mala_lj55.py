#!/usr/bin/env python
"""NTNS-MALA on LJ55 (55 particles, 3D, 165D total).
...
from ntns import LennardJones, ParticleFlowMatching, build_mala_kernel, MALAStepInfo

# Config -----------------------------------------------------------------
N_PARTICLES, N_DIM = 55, 3
DIM = N_PARTICLES * N_DIM  # 165
...
T_DRIFT = 0.5
INIT_STEP_SIZE = 1e-3
RM_TARGET = 0.3   # below MALA 0.574 optimum; trades acceptance for step size
                  # on the NS constrained-prior geometry (non-product, boundary-dominated)
RM_GAMMA = 0.5
RM_KAPPA = 0.0    # constant gain (NS target is non-stationary)

OUTDIR = "results/lj55_mala"
os.makedirs(OUTDIR, exist_ok=True)

# Target + prior ---------------------------------------------------------
target = LennardJones(n_particles=N_PARTICLES, eps=2.0, osc_scale=0.0)
...
flow = ParticleFlowMatching(
    ...
    hidden_nf=EGNN_HIDDEN, n_layers=EGNN_LAYERS, rtol=1e-3, atol=1e-3,
)
